    ...
    def demonstration_clustering(self):
        ...
        if k > 0:
            ...
            for i in range(k):
                ...
                distances = [(np.linalg.norm(embedding - centroid), demo) for (demo, embedding) in cluster_demos]
                # Sort demonstrations by distance
                sorted_demos = [demo for dist, demo in sorted(distances, key=lambda x: x[0])]
                self.clustered_demonstrations[i] = sorted_demos
        else:
    ...
    def adaptive_reasoning_adjustment(self, question, reasoning_chain, m):
        ...
        while loop:
            ...
            for cluster_id, cluster_demos in self.clustered_demonstrations.items():
                similarities = []
                for demo in cluster_demos:
                    demo_text = demo["question"] + "\n" + demo["reasoning"]
                    demo_embedding = self.embedding_model.encode([demo_text])[0]
